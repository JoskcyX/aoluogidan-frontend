        ...
        {rows.map((row) => (
          <div key={row.id} className="border border-line bg-white p-4">
            <p className="font-medium text-ink">
              {row.name} {row.id === currentUserId && <span className="text-xs text-slate">(you)</span>}
            </p>
            <p className="mt-1 truncate text-sm text-slate">{row.email}</p>
            <div className="mt-3 flex flex-wrap items-center gap-2">
              <Badge variant={row.role === "SUPER_ADMIN" ? "default" : "neutral"}>{row.role === "SUPER_ADMIN" ? "Super Admin" : "Editor"}</Badge>
              <Badge variant={row.isActive ? "success" : "neutral"}>{row.isActive ? "Active" : "Disabled"}</Badge>
            </div>
